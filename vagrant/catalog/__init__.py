...
from CreateDB import Base, Company, Employee, Users
...
engine = create_engine(
    'postgresql://catalog2:udacity@localhost/catalog2',
...
def editEmployee(company_id, employee_id):
    if login_session.get('credentials') is None:
        return redirect(url_for('login'))
    editEmployee = session.query(Employee).filter_by(id=employee_id).first()
    if request.method == 'POST':
        if editEmployee.users is None:
            flash("You don't have permission to do this!")
            return redirect(url_for('showCompany', company_id=company_id))
        if editEmployee.users.id != login_session['user_id']:
            flash("You don't have permission to do this!")
            return redirect(url_for('showCompany', company_id=company_id))
...
def deleteEmployee(company_id, employee_id):
    ...
    if request.method == 'POST':
        if employeeToBeDeleted.users is None:
            flash("You don't have permission to do this!")
            return redirect(url_for('showCompany', company_id=company_id))
        if employeeToBeDeleted.users.id != login_session['user_id']:
            flash("You don't have permission to do this!")
            return redirect(url_for('showCompany', company_id=company_id))
...
def getUserID(email):
    users = session.query(Users).filter_by(email=email).first()
...
def createUser(login_session):
    ...
    newUser = Users(name=username, email=email, picture=picture)
    session.add(newUser)
    session.commit()
    users = session.query(Users).filter_by(email=email).first()
    return users.id
